...
Set `config.fill_once=True`, `config.min_mixing=1`, and `config.num_readers=1` to populate the shuffle buffer once, then yield one ordered pass over it without returning indices to the writers. The main loader process waits for the full `shuffle_size` before yielding batches in this mode. Use `loader[buffer_idx]` to read one element by absolute shuffle-buffer index; returned tensors do not include a leading batch dimension.
...
Streaming output batches are views into reusable shared-memory reader buffers, not copies. Their contents may be overwritten after the iterator advances or the batch is released, so callers that need to keep a streaming batch must clone/copy it before requesting another batch or dropping the iterator. With `fill_once=True`, batches and indexed results are direct views into the filled shuffle buffer.
