...
Each Dataloader owns one shared CPU shuffle buffer. The owner writer (`proc_idx=0`) allocates it with `Tensor.share_memory_()` and hosts a local `AF_UNIX` coordinator socket for attachment and queue state.
